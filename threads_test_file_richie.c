...
int main()
{
    ...
    */
    ...
    pthread_t thread_id[8127];

    tfs_read_thread(cenas1);
    for(int i = 0; i < 8127; i++){ 
        //printf("i: %d\n", i);
        if(pthread_create(thread_id + i, NULL, tfs_write_thread , (&new_file_fhandle)) != 0) {return -1;}
    }
                //printf("here read{%d}\n", i);
    // for(int i = 0; i < 20; i++){
    //     if(pthread_create(thread_id + i, NULL, tfs_write_thread , cenas1) != 0){return -1;} 
    // }
    
    // for(int i = 0; i < 10; i++){
    //     if(pthread_create(thread_id + i, NULL, tfs_read_thread , cenas1) != 0){return -1;}
    // }

    for (int i = 0 ; i < 8127; i++){
        if(pthread_join(thread_id[i], NULL) != 0){return -1;}
    }
